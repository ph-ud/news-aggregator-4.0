  ...
  return [
    {
      ...
      description: 'Bring the 4.0-reads sign-in form up in front of the reader and wait while they fill it in. Use this whenever another tool reports that nobody is signed in: reading the library and every write tool need an account. This tool cannot sign anyone in by itself, and it takes no passphrase and no recovery key — the passphrase derives the key that decrypts the library, so it is typed into the page by the person at the keyboard and never passes through an assistant. Ask the reader to switch to this tab. Set mode to signup for a new account, or recover to use a recovery key. email and name only prefill the visible fields. A sign-up ends on a recovery key the reader must write down, and this tool does not return until they have. If the reader has not finished within three minutes the tool returns waiting; check get-account-status again later rather than calling this repeatedly.',
      inputSchema: {
        type: 'object',
        properties: {
